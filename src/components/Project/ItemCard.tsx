...
interface Props {
  ...
  itemType: string;
  setTableData:
    | Dispatch<SetStateAction<ProjectTableData[]>>
    | Dispatch<SetStateAction<FileTableData[]>>;
...
export default function ItemCard({
  ...
  const [collaboratorsArray, setCollaboratorsArray] = useState<string[]>(
    (item as Project).collaborators
  );
  ...
  const secondAttribute =
    itemType === "Project"
      ? (item as Project).projectHostingAlias
      : (item as File).slug;
  ...
  const handleItemOpen = () => {
    return router.push(
      `${itemType === "Project" ? ROUTES.PROJECTS : ROUTES.STUDIO}/${item._id}`
    );
  };
  ...
  const deleteItem = async () => {
    ...
    try {
      ...
      setTableData((prevState: any) =>
        prevState.filter(
          (data: FileTableData) => data._id !== deleteItem?.data?._id
        )
      );
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="max-w-xs mr-[32px] mb-[15px]">
      <Link href={`${ROUTES.STUDIO}/${firstFileId}`}>
        <div>{getImageComponent(item.imageURL)}</div>
      </Link>
      <div
        className={`text-xl font-semibold tracking-tight text-white-700 flex justify-between px-[10px] mt-[12px]`}
      >
        <div>
          ...
          <p className={styles.recentlyEdited}>
            {item?.updatedAt
              ? item.updatedAt
              : "Mahalaxmi Gupta - Edited 2 months ago "}
          </p>
        </div>
        <div>
          <Dropdown
          ...
          >
            ...
            {!sharedProject && (
              <>
                ...
                <Dropdown.Item onClick={deleteItemHandler}>
                  Delete
                </Dropdown.Item>
                <Dropdown.Item onClick={() => duplicateItemHandler(item)}>
                  Duplicate
                </Dropdown.Item>
                {itemType !== "File" && (
                  <Dropdown.Item onClick={handleProjectShare}>
                    Share
                  </Dropdown.Item>
                )}
              </>
