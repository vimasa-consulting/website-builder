...
const ItemListing = ({
  ...
  async function handleEntryDeletion(
    info: CellContext<Project | File, string>
  ) {
    try {
        ...
        <table className="w-full text-black border border-gray-300">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="border border-gray-300 text-xl"
                  >
          ...
          <tbody>
            {table.getRowModel().rows.length ? (
                  ...
                  <tr key={row.id} className="h-60 border-b border-gray-300">
                    {row.getVisibleCells().map((cell) => (
                      <td
                        ...
                        onClick={() => handleTableCellClick(cell)}
                      >
                        <p className="text-center min-w-100 inline-block">
                          {flexRender(
              ...
              <tr className="h-10">
                <td colSpan={3} className="text-center border border-gray-300">
                  No Data Found
                </td>
              </tr>
            )}
