...
const ProfilePopup: React.FC<PopupProps> = ({ closeHandler }) => {
      ...
      <div className="bg-[#1F1F1F] w-[900px] h-[527px] px-[20px] py-[35.5px]  rounded-[25px] shadow-lg relative z-50">
        ...
        <div className="flex gap-[80px]">
          <div className="w-[150px]">
            <p className="font-sans text-xs font-normal mb-[8px] text-white">
              Account
            </p>
            <p
              onClick={() => handleTabChange(Tabs.PROFILE)}
              className={`font-sans text-[15px] font-normal leading-7 tracking-wider mb-[8px] ${
                activeTab === Tabs.PROFILE ? "text-white" : "text-[#888]"
              } cursor-pointer`}
            >
              Profile
            </p>
            <p
              ...
              className={`font-sans text-[15px] font-normal leading-7 tracking-wider mb-[8px] ${
                activeTab === Tabs.SECURITY ? "text-white" : "text-[#888]"
              } cursor-pointer`}
            >
              Security
            </p>
            <p
              onClick={() => handleTabChange(Tabs.TEAM)}
              className={`font-sans text-[15px] font-normal leading-7 tracking-wider mb-[8px] ${
                activeTab === Tabs.TEAM ? "text-white" : "text-[#888]"
              } cursor-pointer`}
            >
              Team
            </p>
            <p
              onClick={() => handleTabChange(Tabs.BILLING)}
              className={`font-sans text-[15px] font-normal leading-7 tracking-wider mb-[8px] ${
                activeTab === Tabs.BILLING ? "text-white" : "text-[#888]"
              } cursor-pointer`}
            >
              Plan & Billings
            </p>
